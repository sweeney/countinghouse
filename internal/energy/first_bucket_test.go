...
// ---- BuildSeries: counter path ---------------------------------------------

// TestBuildSeriesCounterQueriesTheClippedHead pins the counter-path mechanism.
// aggregateWindow buckets on the GRID, so bucket 0's difference() delta is the
// whole grid interval and win.Start never enters the arithmetic. The fix is a
// dedicated exact-range query for the partial head, substituted for bucket 0.
func TestBuildSeriesCounterQueriesTheClippedHead(t *testing.T) {
	loc := mustLondon(t)
	win := Window{
		Start: time.Date(2026, 6, 11, 14, 29, 0, 0, loc),
		Stop:  time.Date(2026, 6, 11, 16, 0, 0, 0, loc),
	...
	// The padded series query reports a FULL grid interval for bucket 0 (0.5),
	// of which only the last minute is inside the window.
	var seriesRows []influx.Row
	for _, b := range buckets {
		seriesRows = append(seriesRows, influx.Row{DeviceID: "winefridge", Field: "energy_kwh", Value: 0.5, Time: b})
	}
	const headKWh = 0.017 // what actually accrued in 14:29->14:30
	...
	q := &influx.FakeQuerier{QueryFunc: func(flux string) ([]influx.Row, error) {
		...
		headFlux = flux
		return []influx.Row{{DeviceID: "winefridge", Field: "energy_kwh", Value: headKWh, Time: buckets[1]}}, nil
	}}
	...
	if headFlux == "" {
		t.Fatal("no exact-range head query was issued for the partial first bucket")
	}
	// It must span exactly [win.Start, buckets[1]) — the in-window part of bucket 0.
	if want := win.Start.UTC().Format(time.RFC3339); !strings.Contains(headFlux, "range(start: "+want+",") {
		t.Errorf("head query does not start at win.Start (%s):\n%s", want, headFlux)
	}
	if want := buckets[1].UTC().Format(time.RFC3339); !strings.Contains(headFlux, "stop: "+want+")") {
		t.Errorf("head query does not stop at the second bucket start (%s):\n%s", want, headFlux)
	}

	wine := resp.Series[0]
	if wine.KWh[0] != headKWh {
		t.Errorf("winefridge kwh[0] = %v, want %v (the clipped head, not the full grid bucket)", wine.KWh[0], headKWh)
	}
	for i := 1; i < len(buckets); i++ {
		if wine.KWh[i] != 0.5 {
			t.Errorf("winefridge kwh[%d] = %v, want 0.5 (interior buckets unchanged)", i, wine.KWh[i])
		}
	}
}

// TestBuildSeriesCounterNoHeadQueryWhenGridAligned guards the cost of the fix:
// today/week/month/<N>d all start on the grid, and must not pay for a fourth
// query.
func TestBuildSeriesCounterNoHeadQueryWhenGridAligned(t *testing.T) {
	loc := mustLondon(t)
	win := Window{
		Start: time.Date(2026, 6, 11, 0, 0, 0, 0, loc),
	...
	if _, err := BuildSeries(context.Background(), q, "statehouse", win, iv, GroupByDevice, false, false, devices, testTariff(), nil, loc); err != nil {
		t.Fatalf("BuildSeries: %v", err)
	}
	for _, flux := range q.Queries {
		if strings.Contains(flux, "energy_kwh") && !strings.Contains(flux, "aggregateWindow") {
			t.Errorf("grid-aligned window issued a head query it does not need:\n%s", flux)
		}
	}
	if len(q.Queries) != 2 {
		t.Errorf("query count = %d, want 2 (counter series + mean power)", len(q.Queries))
	}
}

// TestBuildSeriesCounterHeadWithNoDataZeroesBucketZero: the head query is
// authoritative for bucket 0. A device whose counter did not move inside the
// partial head must report 0 there, NOT the padded query's full-grid delta.
func TestBuildSeriesCounterHeadWithNoDataZeroesBucketZero(t *testing.T) {
	loc := mustLondon(t)
	win := Window{
		Start: time.Date(2026, 6, 11, 14, 29, 0, 0, loc),
		Stop:  time.Date(2026, 6, 11, 16, 0, 0, 0, loc),
	...
	q := &influx.FakeQuerier{QueryFunc: func(flux string) ([]influx.Row, error) {
		...
		return nil, nil // head query: no samples in the partial head
	}}
	resp, err := BuildSeries(context.Background(), q, "statehouse", win, iv, GroupByDevice, false, false, devices, testTariff(), nil, loc)
	if err != nil {
		t.Fatalf("BuildSeries: %v", err)
	}
	if got := resp.Series[0].KWh[0]; got != 0 {
		t.Errorf("winefridge kwh[0] = %v, want 0 (no counter movement inside the clipped head)", got)
	}
}
